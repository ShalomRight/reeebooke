...
const COLORS: TBookingColor[] = ["blue", "green", "red", "yellow", "purple", "orange", "gray"];
...
const mockGenerator = (numberOfBookings: number): IBooking[] => {
  ...
  // Create an booking happening now
  ...
  // Only add the current booking if it's not on September 20th
  if (now.getMonth() !== 8 || now.getDate() !== 20) {
    // Month is 0-indexed (8 = September)
    result.push(currentBooking);
  }

  // Generate the remaining bookings
  let i = 0;
  let attempts = 0;
  const maxAttempts = numberOfBookings * 3; // Prbooking infinite loop with a reasonable max attempts

  while (i < numberOfBookings - 1 && attempts < maxAttempts) {
    attempts++;

    // Determine if this is a multi-day booking (10% chance)
    ...
    if (isMultiDay) {
      // Multi-day booking: Add 1-4 days
      const additionalDays = Math.floor(Math.random() * 4) + 1;
      endDate.setDate(startDate.getDate() + additionalDays);

      // Ensure multi-day bookings don't cross September 20th
      const endMonth = endDate.getMonth();
      const endDay = endDate.getDate();
      const startMonth = startDate.getMonth();
      const startDay = startDate.getDate();

      // Check if booking spans across September 20th
      if (
...
export const CALENDAR_ITEMS_MOCK: IBooking[] = mockGenerator(80);
