...
import { YearViewDayCell } from "@/components/admin/calendar/components/year-view/year-view-day-cell";
...
export function YearViewMonth({ month, bookings }: IProps) {
      ...
      <div className="flex-1 space-y-2 rounded-b-lg border border-t-0 p-3">
        ...
        <div className="grid grid-cols-7 gap-x-0.5 gap-y-2">
          {daysInMonth.map((day, index) => {
            ...
            const dayBookings = bookings.filter(booking => isSameDay(parseISO(booking.startDate), date) || isSameDay(parseISO(booking.endDate), date));

            return <YearViewDayCell key={`day-${day}`} day={day} date={date} bookings={dayBookings} />;
          })}
        </div>
      </div>
    </div>
