...
function ZapBookingFormContent() {
	...
	const {
		...
		dailyCapacities, totalPrice, selectedServiceData, isLoadingTimeSlots, isLoadingServices,
	} = form
	...
	const allSlotsBooked = !!selectedDate && !!selectedService
		&& !isLoadingTimeSlots
		&& timeSlots.length > 0
		&& timeSlots.every(t => !t.available)
					...
					<div className="animate-in fade-in slide-in-from-right-4 duration-300">
						...
						<div className="max-w-sm mx-auto">
							<CalendarCard
								...
								disabledDates={
									selectedService
										? Object.keys(dailyCapacities).filter(d => dailyCapacities[+d].total > 0 && dailyCapacities[+d].available === 0).map(Number)
										: []
								}
							/>
						</div>
...
export function ZapBookingForm() {
	return (
		<Suspense
			fallback={
